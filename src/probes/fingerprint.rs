...
pub fn fingerprint_tech(
    headers: &HeaderMap,
    body: &str,
    _status_code: u16,
) -> Vec<TechnologyDetection> {
    ...
    if let Some(server) = headers.get("server").and_then(|v| v.to_str().ok())
        && (server.to_lowercase().contains("github.com")
            || server.to_lowercase().contains("github.io"))
    {
    ...
    {
        let version = server.split('/').nth(1).map(|v| v.trim().to_string());
        detections.push(TechnologyDetection {
    ...
    // 5. ASP.NET / Kestrel
    if let Some(server) = headers.get("server").and_then(|v| v.to_str().ok())
        && server.to_lowercase().contains("kestrel")
    {
        detections.push(TechnologyDetection {
    ...
    if body_lower.contains("timestamp")
    ...
    // 8. Varnish Cache
    if let Some(server) = headers.get("server").and_then(|v| v.to_str().ok())
        && server.to_lowercase().contains("varnish")
    {
        detections.push(TechnologyDetection {
    ...
    if let Some(server) = headers.get("server").and_then(|v| v.to_str().ok())
        && server.to_lowercase().contains("atlassianedge")
    {
        detections.push(TechnologyDetection {
    ...
    if let Some(powered_by) = headers.get("x-powered-by").and_then(|v| v.to_str().ok())
        && powered_by.to_lowercase().contains("express")
    {
        detections.push(TechnologyDetection {
