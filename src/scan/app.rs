...
pub async fn run(args: Args) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.ai_analyze {
        ...
        if let Some(provider) = provider {
            match crate::analyst::NetworkAiProvider::new(
                provider,
                Duration::from_secs(args.ai_timeout_seconds),
            ) {
                Ok(provider) => {
                    let config = crate::analyst::AnalystConfig {
                        max_candidates: args.ai_max_candidates.min(10),
                        max_related_endpoints: args.ai_max_related_endpoints.min(40),
                        max_requests: args.ai_max_requests,
                        max_input_bytes: args.ai_max_input_bytes,
                        max_output_bytes: args.ai_max_output_bytes,
                        retries: args.ai_retries,
                        output_dir: std::path::PathBuf::from(&args.triage_dir),
                    };
                    match crate::analyst::run(&conn, scan_run.id, &provider, &config).await {
                        Ok(result) => println!(
                            "🤖 Phase 6 analysis saved for {} ranked candidate(s).",
                            result.candidate_analyses.len()
                        ),
                        Err(error) => eprintln!(
                            "⚠️  Phase 6 AI analysis unavailable; deterministic outputs remain valid: {}",
                            error.0
                        ),
                    }
                }
                Err(error) => eprintln!("⚠️  Phase 6 provider unavailable: {}", error.0),
            }
        } else {
            eprintln!("⚠️  Phase 6 skipped: ANTHROPIC_API_KEY is missing.");
        }
    }
